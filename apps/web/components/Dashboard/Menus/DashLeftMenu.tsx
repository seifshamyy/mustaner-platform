...
function DashLeftMenu() {
  ...
  return (
    ...
    <nav
      ...
      className={cn(
        "flex flex-col text-neutral-900 h-screen sticky top-0 z-overlay border-e border-neutral-200 bg-white transition-all duration-300",
        isCollapsed ? "w-[72px]" : "w-64"
      )}
    >
      {/* Header with Logo and Toggle */}
      <div className={cn(
        "relative flex items-center h-16 border-b border-neutral-200 px-4 shrink-0",
        ...
        >
          {isCollapsed ? (
            <img src="/brand/mark.png" alt={org?.name ?? 'Mustaner'} className="h-9 w-9 rounded-[3px]" />
          ) : org?.logo_image ? (
            <img
            ...
            <img src="/brand/wordmark.svg" alt={org?.name ?? 'Mustaner'} className="h-7 w-auto" />
          )}
        </Link>
        ...
        {/* Onboarding progress reuses this header's bottom border as its track,
            filled with the wordmark's green → teal → blue. */}
        {showOnboarding && (
          <>
            {/* faint full-width track so the border reads as a track even at 0% */}
            <div className="absolute -bottom-px start-0 end-0 h-[2px] bg-blue-600/10" />
            <motion.div
              className="absolute -bottom-px start-0 h-[2px]"
              style={{
        ...
        <AdminAuthorization authorizationMode="component">
          <div className="space-y-1">
            ...
            <HoverMenu
              content={
                <HoverMenuContent className="w-64">
                  <HoverMenuLabel className="text-neutral-600 font-medium">{t('courses.courses')}</HoverMenuLabel>
                  <HoverMenuSeparator />
                  <HoverMenuItem asChild>
                    <Link href="/dash/courses" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                  ...
                  {recentCourses.length > 0 && (
                    <>
                      <HoverMenuSeparator />
                      <HoverMenuLabel className="text-neutral-400">{t('common.recent')}</HoverMenuLabel>
                      {recentCourses.map((course: any) => (
                        <HoverMenuItem key={course.course_uuid} asChild>
                          <Link
                            href={`/dash/courses/course/${course.course_uuid.replace('course_', '')}/settings`}
                            className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors"
                          >
                            <PencilSimple size={14} className="text-neutral-400" />
                ...
                return (
                  ...
                  >
                    ...
                    <span className="relative flex items-center justify-center">
                      ...
                      {isCollapsed && (
                        <CaretDown aria-hidden="true" size={8} weight="bold" className={cn("absolute -end-2.5", active ? "text-neutral-500" : "text-neutral-400")} />
                      )}
                    </span>
                    {!isCollapsed && (
                      <>
                        <span className="text-sm font-medium flex-1 text-start">{t('courses.courses')}</span>
                        <CaretDown aria-hidden="true" size={14} weight="bold" className={active ? "text-neutral-600" : "text-neutral-400"} />
                      </>
                    )}
                  </Link>
                )
            ...
            <HoverMenu
              content={
                <HoverMenuContent className="w-72">
                  <HoverMenuLabel className="text-neutral-600 font-medium">{t('common.assignments')}</HoverMenuLabel>
                  <HoverMenuSeparator />
                  <HoverMenuItem asChild>
                    <Link href="/dash/assignments" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                        ...
                        <HoverMenuItem key={assignment.assignment_uuid} asChild>
                          ...
                          >
                            ...
                            <div className="flex flex-col min-w-0">
                              ...
                              <span className="text-xs text-neutral-400 truncate">{assignment.courseName}</span>
                            </div>
                          </Link>
                        </HoverMenuItem>
                      ))}
            ...
            <HoverMenu
              content={
                <HoverMenuContent className="w-64">
                  <HoverMenuLabel className="text-neutral-600 font-medium">{t('common.users')}</HoverMenuLabel>
                  <HoverMenuSeparator />
                  <HoverMenuItem asChild>
                    <Link href="/dash/users/settings/users" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                      <Users size={16} weight="fill" />
                      <span>{t('dashboard.users.settings.tabs.users')}</span>
                    </Link>
                  </HoverMenuItem>
                  <HoverMenuItem asChild>
                    <Link href="/dash/users/settings/usergroups" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                  ...
                  <HoverMenuItem asChild>
                    <Link href="/dash/users/settings/roles" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                  ...
                  <HoverMenuItem asChild>
                    <Link href="/dash/users/settings/signups" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                      <ClipboardText size={16} weight="fill" />
                      <span>{t('dashboard.users.settings.tabs.signups')}</span>
                    </Link>
                  </HoverMenuItem>
                  <HoverMenuItem asChild>
                    <Link href="/dash/users/settings/add" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
            ...
            <HoverMenu
              content={
                <HoverMenuContent className="w-64">
                  <HoverMenuLabel className="text-neutral-600 font-medium">{t('common.organization')}</HoverMenuLabel>
                  <HoverMenuSeparator />
                  <HoverMenuItem asChild>
                    <Link href="/dash/org/settings/general" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                      <Gear size={16} weight="fill" />
                      <span>{t('dashboard.organization.settings.tabs.general')}</span>
                    </Link>
                  </HoverMenuItem>
                  <HoverMenuItem asChild>
                    <Link href="/dash/org/settings/branding" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                      <Palette size={16} weight="fill" />
                      <span>{t('dashboard.organization.settings.tabs.branding')}</span>
                    </Link>
                  </HoverMenuItem>
                  <HoverMenuItem asChild>
                    <Link href="/dash/org/settings/landing" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                      <Rocket size={16} weight="fill" />
                      <span>{t('dashboard.organization.settings.tabs.landing')}</span>
                    </Link>
                  </HoverMenuItem>
                  <HoverMenuItem asChild>
                    <Link href="/dash/org/settings/ai" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                  ...
                  {canManageOrg && (
                    <HoverMenuItem asChild>
                      <Link href="/dash/org/settings/usage" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                  ...
                  <HoverMenuItem asChild>
                    <Link href="/dash/org/settings/other" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
            ...
            <HoverMenu
              content={
                <HoverMenuContent className="w-64">
                  <HoverMenuLabel className="text-neutral-600 font-medium">{t('dashboard.developers.breadcrumb', { defaultValue: 'Developers' })}</HoverMenuLabel>
                  <HoverMenuSeparator />
                  <HoverMenuItem asChild>
                    <Link href="/dash/developers/api" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                      <Key size={16} weight="fill" />
                      <span className="flex items-center">{t('dashboard.organization.settings.tabs.api', { defaultValue: 'API Access' })}<PlanBadge currentPlan={plan} requiredPlan="pro" variant="light" /></span>
                    </Link>
                  </HoverMenuItem>
                  <HoverMenuItem asChild>
                    <Link href="/dash/developers/automations" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                      <Lightning size={16} weight="fill" />
                      <span className="flex items-center">{t('dashboard.organization.settings.tabs.automations', { defaultValue: 'Automations' })}<PlanBadge currentPlan={plan} requiredPlan="pro" variant="light" /></span>
                    </Link>
                  </HoverMenuItem>
                  <HoverMenuItem asChild>
                    <Link href="/dash/developers/domains" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                      <LinkSimple size={16} weight="fill" />
                      <span className="flex items-center">{t('dashboard.organization.settings.tabs.domains', { defaultValue: 'Domains' })}<PlanBadge currentPlan={plan} requiredPlan="standard" variant="light" /></span>
                    </Link>
                  </HoverMenuItem>
                  <HoverMenuItem asChild>
                    <Link href="/dash/developers/seo" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                      <MagnifyingGlass size={16} weight="fill" />
                      <span>SEO</span>
                    </Link>
                  </HoverMenuItem>
                  <HoverMenuItem asChild>
                    <Link href="/dash/developers/sso" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                      <Lock size={16} weight="fill" />
                      <span className="flex items-center">{t('dashboard.organization.settings.tabs.sso', { defaultValue: 'SSO' })}<PlanBadge currentPlan={plan} requiredPlan="enterprise" variant="light" /></span>
                    </Link>
                  </HoverMenuItem>
                </HoverMenuContent>
              }
            ...
            <HoverMenu
              content={
                <HoverMenuContent className="w-64">
                  <HoverMenuLabel className="text-neutral-600 font-medium">Analytics</HoverMenuLabel>
                  <HoverMenuSeparator />
                  <HoverMenuItem asChild>
                    <Link href="/dash/analytics" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                      <ChartBar size={16} weight="fill" />
            ...
            {/* Integrated apps: the website editor and the org's own tools, for the people who run the org */}
            {canManageOrg && (
              <MenuLink
                href="/dash/apps"
                icon={<SquaresFour size={20} weight="fill" />}
                label={t('dashboard.integrated_apps.title')}
                isCollapsed={isCollapsed}
                active={isActivePath('/dash/apps')}
              />
            )}
            ...
            {(!showCommunities || !showPodcasts || !showBoards || !showPlaygrounds || !showPayments) && (
              <HoverMenu
                content={
                  <HoverMenuContent className="w-64">
                    <HoverMenuLabel className="flex items-center justify-between text-neutral-600 font-medium">
                      <span>{t('common.other')}</span>
                      <span className="text-[9px] font-medium uppercase tracking-wider px-1.5 py-0.5 rounded bg-neutral-100 text-neutral-300">
                        {t('common.disabled')}
                      </span>
                    </HoverMenuLabel>
                    <HoverMenuSeparator />
                    {!showCommunities && (
                      <HoverMenuItem asChild>
                        <Link href="/dash/communities" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-400 hover:text-neutral-500 hover:bg-neutral-50 cursor-pointer transition-colors">
                    ...
                    {!showPodcasts && (
                      <HoverMenuItem asChild>
                        <Link href="/dash/podcasts" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-400 hover:text-neutral-500 hover:bg-neutral-50 cursor-pointer transition-colors">
                    ...
                    {!showBoards && (
                      <HoverMenuItem asChild>
                        <Link href="/dash/boards" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-400 hover:text-neutral-500 hover:bg-neutral-50 cursor-pointer transition-colors">
                    ...
                    {!showPlaygrounds && (
                      <HoverMenuItem asChild>
                        <Link href="/dash/playgrounds" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-400 hover:text-neutral-500 hover:bg-neutral-50 cursor-pointer transition-colors">
                    ...
                    {!showPayments && (
                      <HoverMenuItem asChild>
                        <Link href="/dash/payments/overview" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-400 hover:text-neutral-500 hover:bg-neutral-50 cursor-pointer transition-colors">
              ...
              >
                <button
                  ...
                  className={cn(
                    "flex items-center w-full rounded-lg text-neutral-400 hover:text-neutral-500 hover:bg-neutral-50 transition-all",
                    isCollapsed ? "justify-center h-10" : "px-3 py-2 gap-3"
                  )}
                >
                  <span className="relative flex items-center justify-center">
                    ...
                    {isCollapsed && (
                      <CaretDown aria-hidden="true" size={8} weight="bold" className="absolute -end-2.5 text-neutral-300" />
                    )}
                  </span>
                  {!isCollapsed && (
                    <>
                      <span className="text-sm font-medium flex-1 text-start">{t('common.other')}</span>
                      <CaretDown aria-hidden="true" size={14} weight="bold" className="text-neutral-300" />
                    </>
                  )}
                </button>
              </HoverMenu>
      ...
      {multiOrg && plan === 'free' && !isCollapsed && canManageOrg && (
        ...
        >
          ...
          <motion.div layout className="relative">
            ...
            <span className="inline-flex items-center px-1.5 py-0.5 rounded-md bg-neutral-100 text-neutral-500 text-[8px] font-bold uppercase tracking-wider">
              {t('plan.free_plan_title', { defaultValue: 'Free plan' })}
            </span>
            <p className="mt-2 text-[13px] font-bold text-neutral-900 leading-tight">
            ...
            <motion.a
              ...
              className="mt-3 relative overflow-hidden flex items-center justify-center gap-1.5 w-full rounded-lg bg-blue-600 text-white text-[13px] font-semibold py-2"
            >
      ...
      {/* Bottom Section */}
      <div className="border-t border-neutral-200 py-3 px-3 shrink-0">
        <div className="space-y-1">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  onClick={toggleCollapse}
                  className="flex items-center justify-center w-full h-10 rounded-lg text-neutral-400 hover:text-neutral-900 hover:bg-neutral-100 transition-all"
                >
                  <SidebarSimple size={20} weight="fill" />
                </button>
              </TooltipTrigger>
              <TooltipContent side="right" className="z-tooltip bg-neutral-900 border-neutral-800 text-white text-xs px-2 py-1 shadow-lg shadow-black/20">
                {t('common.expand')}
              </TooltipContent>
            </Tooltip>
          )}
          ...
          <HoverMenu
            ...
            content={
              <HoverMenuContent className="w-64 max-h-96 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                <HoverMenuLabel className="flex items-center gap-2 text-neutral-600 font-medium">
                ...
                {AVAILABLE_LANGUAGES.map((language) => (
                  <HoverMenuItem
                    ...
                    className="flex items-center justify-between px-3 py-2.5 cursor-pointer text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 transition-colors"
                  >
                    <div className="flex flex-col">
                      <span className="font-medium text-sm">{language.nativeName}</span>
                      <span className="text-xs text-neutral-400">{t(language.translationKey)}</span>
                    </div>
          ...
          <HoverMenu
            ...
            content={
              <HoverMenuContent className="w-56">
                ...
                <HoverMenuItem asChild>
                  <a href={`mailto:${SUPPORT_EMAIL}`} className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
            ...
            <HoverMenu
              ...
              content={
                <HoverMenuContent className="w-64 max-h-96 overflow-y-auto">
                  ...
                  <HoverMenuSeparator />
                  <HoverMenuItem asChild>
                    <a href={getMainDomainUri('/home')} className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                  ...
                  {canManageOrg && (
                    <HoverMenuItem asChild>
                      <a href={getMainDomainUri(`/billing?org=${org?.slug ?? ''}`)} className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                        <CurrencyCircleDollar size={16} weight="fill" />
                  ...
                  {myOrgs.map((o: any) => (
                    <HoverMenuItem key={o.id} asChild>
                      <a href={getUriWithOrg(o.slug, '/')} className={cn(
                        "flex items-center gap-2 px-3 py-2 text-sm hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors",
                        o.id === org?.id ? "text-neutral-900" : "text-neutral-600"
                      )}>
                        <Buildings size={16} weight="fill" />
                  ...
                  <HoverMenuSeparator />
                  <HoverMenuItem asChild>
                    <a href={getMainDomainUri('/new')} className="flex items-center gap-2 px-3 py-2 text-sm font-semibold text-neutral-800 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
            ...
            content={
              <HoverMenuContent className="w-56">
                <div className="px-3 py-2">
                  <p className="text-sm font-semibold text-neutral-800">{session?.data?.user?.username}</p>
                  <p className="text-xs text-neutral-400">{session?.data?.user?.email}</p>
                </div>
                <HoverMenuSeparator />
                <HoverMenuItem asChild>
                  <Link href="/account/general" className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                    <Gear size={16} weight="fill" />
                    <span>{t('common.settings')}</span>
                  </Link>
                </HoverMenuItem>
                <HoverMenuItem asChild>
                  <Link href={getUriWithOrg(org?.slug, '/account/purchases')} className="flex items-center gap-2 px-3 py-2 text-sm text-neutral-600 hover:text-neutral-900 hover:bg-neutral-100 cursor-pointer transition-colors">
                ...
                <HoverMenuItem
                  onClick={() => logOutUI()}
                  className="flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:text-red-600 hover:bg-neutral-100 cursor-pointer transition-colors"
                >
            ...
            <button className={cn(
              ...
              {!isCollapsed && (
                <div className="flex flex-col min-w-0 flex-1 text-start">
                  <span className="text-sm font-medium truncate text-neutral-800">{session?.data?.user?.username}</span>
                  <span className="text-xs text-neutral-400 truncate">{session?.data?.user?.email}</span>
                </div>
              )}
            </button>
          </HoverMenu>
