...
function Watermark() {
    ...
    return (
        <div className='fixed bottom-8 end-8 z-50'>
            <Link href={getSiteUrl() ?? '/'} className="flex items-center cursor-pointer bg-white/80 backdrop-blur-lg text-gray-700 rounded-2xl p-2 light-shadow text-xs px-5 font-semibold space-x-2">
                <p>{t('common.made_with')}</p>
                <Image unoptimized src={wordmark} alt="Mustaner" quality={100} width={80} />
            </Link>
        </div>
    )
}
